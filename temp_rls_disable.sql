-- GEÇICI RLS FIX - Supabase Dashboard'da çalıştırın

-- 1. Önce mevcut tabloyu kontrol et ve eksik sütunları ekle
ALTER TABLE student_question_stats 
ADD COLUMN IF NOT EXISTS program_id UUID DEFAULT '00000000-0000-0000-0000-000000000000';
...
-- 3. Mevcut unique constraint'i kaldır ve yenisini ekle
DROP INDEX IF EXISTS student_question_stats_unique;
...
DROP TRIGGER IF EXISTS update_student_question_stats_updated_at ON student_question_stats;
